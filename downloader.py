...
class BinanceDownloader:
    ...
    def download(self):
        for path in self.paths:
            self._download(self.BINANCE_DATA_URL, [], 0, path, self.xpaths)
        if self.executor:
            self.executor.shutdown()

    def _download(self, url, components, level, path, xpaths):
        items = self._fetch(url)
        for item in items:
            if not path.match(item.name, level):
                continue
            ...
            if item.is_file():
                ...
                self._download(item.url, components, level + 1, path, cur_xpaths)
                components.pop()
